...
export async function fetchTickets({ force = false } = {}) {
    if (!force) {
    ...
    if (!messages.length) {
        await updateTicketCache([]);
        return [];
    }

    const fullMessages = await Promise.all(messages.map(msg => gmail.users.messages.get({
        userId: GMAIL_INBOX_USER,
        id: msg.id,
        format: 'metadata',
    ...
    const ticketsWithStatus = await applyStatuses(parsed);
    await updateTicketCache(ticketsWithStatus);
    return ticketsWithStatus;
}

export async function forceRefreshTickets() {
    return fetchTickets({ force: true });
}
...
async function assertHasCredentials(client) {
    ...
    if (!credentials.access_token && !credentials.refresh_token) {
        throw new Error('Authorize Gmail access to load tickets.');
    }
}
